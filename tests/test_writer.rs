...
fn test_record_to_string_for_data_record() {
  ...
  assert_eq!(empty_data_record.to_string(), Ok(String::from(":0000000000")));
  ...
  assert_eq!(populated_data_record.to_string(), Ok(String::from(":0B0010006164647265737320676170A7")));
...
fn test_record_to_string_for_data_record_with_invalid_data() {
  ...
  assert_eq!(invalid_data_record.to_string(), Err(WriterError::DataExceedsMaximumLength(256)));
}
...
fn test_record_to_string_for_esa_record() {
  ...
  assert_eq!(esa_record_1.to_string(), Ok(String::from(":020000021200EA")));

  let esa_record_2 = Record::ExtendedSegmentAddress(0x55AA);
  assert_eq!(esa_record_2.to_string(), Ok(String::from(":0200000255AAFD")));
}

#[test]
fn test_record_to_string_for_ssa_record() {
  let ssa_record_1 = Record::StartSegmentAddress { cs: 0x0110, ip: 0x3801 };
  assert_eq!(ssa_record_1.to_string(), Ok(String::from(":0400000301103801AF")));

  let ssa_record_2 = Record::StartSegmentAddress { cs: 0x0000, ip: 0x3800 };
  assert_eq!(ssa_record_2.to_string(), Ok(String::from(":0400000300003800C1")));
}

#[test]
fn test_record_to_string_for_ela_record() {
  let ela_record_1 = Record::ExtendedLinearAddress(0xFFFF);
  assert_eq!(ela_record_1.to_string(), Ok(String::from(":02000004FFFFFC")));

  let ela_record_2 = Record::ExtendedLinearAddress(0x0F55);
  assert_eq!(ela_record_2.to_string(), Ok(String::from(":020000040F5596")));
}

#[test]
fn test_record_to_string_for_sla_record() {
  let sla_record_1 = Record::StartLinearAddress(0x000000CD);
  assert_eq!(sla_record_1.to_string(), Ok(String::from(":04000005000000CD2A")));

  let sla_record_2 = Record::StartLinearAddress(0x11223344);
  assert_eq!(sla_record_2.to_string(), Ok(String::from(":04000005112233444D")));
}

#[test]
fn test_create_object_file_representation_incorrect_termination() {
  assert_eq!(create_object_file_representation(&[]), Err(WriterError::MissingEndOfFileRecord));
  assert_eq!(create_object_file_representation(&[Record::ExtendedLinearAddress(0)]), Err(WriterError::MissingEndOfFileRecord));
  assert_eq!(create_object_file_representation(&[Record::EndOfFile, Record::ExtendedLinearAddress(0)]), Err(WriterError::MissingEndOfFileRecord));
}
...
fn test_create_object_file_representation_eof_only() {
  ...
  assert_eq!(create_object_file_representation(records), Ok(expected_result));
}
